...
function Read() {
  nameListDOM.innerHTML = ""; // Temizleme
      ...
      for (let i = 0; i < getUserList.length; i++) {
        ...
        <div class="user-item">
          ...
          <div class="buttons">
            ...
            <button class="danger" onclick="Delete(${i})"><i class="fas fa-trash"></i>Delete</button>
          </div>
        </div>`;
      }
    }
  ...
  for (let i = 0; i < editUsers.length; i++) {
    if (i == item) {
      ...
      <div class="buttons">
        ...
        <button class="warning" onclick="Read()"><i class="fas fa-trash"></i>Cancel</button>
      </div>
    </div>`;
    } else {
      nameListDOM.innerHTML += `
...
document.addEventListener("DOMContentLoaded", () => {
  Read();
}); // burada Read fonksiyonunun HTML yüklendikten sonra çalışmasını sağladık.
